...
/*   Updated: 2022/12/12 15:49:47 by gbohm            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void mlx_set_uniform_1d(mlx_t *mlx, const char *key, double x)
{
	mlx_ctx_t* mlxctx = mlx->context;

	glUniform1d(glGetUniformLocation(mlxctx->shaderprogram, key), x);
}

void mlx_set_uniform_2d(mlx_t *mlx, const char *key, double x, double y)
{
	mlx_ctx_t* mlxctx = mlx->context;

	glUniform2d(glGetUniformLocation(mlxctx->shaderprogram, key), x, y);
}

void mlx_set_uniform_1f(mlx_t *mlx, const char *key, float x)
{
	mlx_ctx_t* mlxctx = mlx->context;

	glUniform1f(glGetUniformLocation(mlxctx->shaderprogram, key), x);
}

void mlx_set_uniform_2f(mlx_t *mlx, const char *key, float x, float y)
{
	mlx_ctx_t* mlxctx = mlx->context;

	glUniform2f(glGetUniformLocation(mlxctx->shaderprogram, key), x, y);
}

void mlx_set_uniform_1i(mlx_t *mlx, const char *key, int x)
{
	mlx_ctx_t* mlxctx = mlx->context;

	glUniform1i(glGetUniformLocation(mlxctx->shaderprogram, key), x);
}
